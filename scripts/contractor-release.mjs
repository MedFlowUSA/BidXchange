...
const files = validateMigrations('supabase/migrations').filter(
  (m) => m.file >= (target === 'staging' ? '20260921001700' : '20260921002000'),
);
let db;
try {
  ...
  await db.query('select pg_advisory_xact_lock(20260921,20)');
  const installed = (
    await db.query('select version,statements from supabase_migrations.schema_migrations')
  ).rows;
  for (const version of ['20260919000100', '20260920001100', '20260921001500', '20260921001600'])
    assert(
  ...
  console.error(
    'Contractor migration failed and transaction rolled back. Code: ' +
