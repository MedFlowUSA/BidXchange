...
const target = path.join(root, '.tmp', 'release-workflow-staging');
...
try {
  ...
  for (let i = 0; i < 60; i++) {
    try {
      if ((await fetch('http://127.0.0.1:3102/login')).ok) {
        ready = true;
        break;
      }
    } catch {}
  ...
  await new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      ['scripts/staging/release-browser.mjs', 'http://127.0.0.1:3102'],
      {
