...
const files = validateMigrations('supabase/migrations').filter(
  (m) => m.file === '20260922003000_company_information_requests.sql',
);
let db;
try {
  ...
  await db.query('select pg_advisory_xact_lock(20260922,30)');
  const installed = (
  ...
  console.error(
    'Information request migration failed and transaction rolled back. Code: ' +
