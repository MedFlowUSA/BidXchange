...
const files = validateMigrations('supabase/migrations').filter(
  (m) => m.file === '20260922002900_requirement_lifecycle.sql',
);
let db;
try {
  ...
  await db.query('select pg_advisory_xact_lock(20260922,29)');
  const installed = (
    await db.query('select version,statements from supabase_migrations.schema_migrations')
  ).rows;
  for (const version of ['20260919000100', '20260921001500', '20260921002100', '20260922002800'])
    assert(
  ...
  console.error(
    'Requirement lifecycle migration failed and transaction rolled back. Code: ' +
