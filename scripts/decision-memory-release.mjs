...
const files = validateMigrations('supabase/migrations').filter(
  (m) => m.file === '20260922003100_decision_memory.sql',
);
assert.equal(files.length, 1, 'Reviewed migration 031 must be present');
let db;
try {
  ...
  await db.query('select pg_advisory_xact_lock(20260922,31)');
  const installed = (
  ...
  console.error(
    'Decision memory migration failed and transaction rolled back. Code: ' +
