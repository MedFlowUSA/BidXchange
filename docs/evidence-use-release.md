...
The user explicitly approved production migrations 007?009 and evidence-review activation. All three migrations are installed in production and permission checks passed. The approved release sets `BIDXCHANGE_EVIDENCE_REVIEWS_ENABLED=true` in `vercel.json`; local development remains disabled unless configured. A local isolated app uses the hosted staging database for signed-in browser validation.
...
Staging already has these migrations. Forward corrections preserve its applied migration history. `scripts/staging/migrations.json` pins all files using SHA-256 over UTF-8 with LF endings; staging package revision 5 excludes the real-company seed. `scripts/evidence-use-release.mjs migrate-staging` checks the target and applied SQL before changing anything. The approved `migrate-production` mode verifies the production project reference and host, rejects test database overrides, and checks grants before commit.
...
The existing named-production-migration approval gate is documented in `docs/demo-intake-release.md`: broad permission did not satisfy the earlier automatic approval review. That explicit approval has now been received for these three migrations and the evidence-review feature flag. No fresh production attempt has been rejected in this release.
...
Operational rollback: set the evidence-review flag to `false` in `vercel.json` and redeploy. Preserve review history and the stricter fact-verification trigger; do not drop the table or restore permissive grants as an application rollback. The schema adds foreign keys that prevent deleting referenced facts, requirements or reviewers while review history exists. No existing business records are rewritten by these migrations.
