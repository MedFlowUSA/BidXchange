...
Released with the user-authorized audit UI deployment `dpl_F52ZeUbruvnKN2kkvTQ61PYNc36d` at `https://bidxapp.vercel.app`. Production AI remains disabled. Dedicated staging now exists and the current release passed authenticated viewer readiness/navigation checks there; the six detailed record-retrieval regressions remain local mocked tests rather than new hosted deep-link tests.
