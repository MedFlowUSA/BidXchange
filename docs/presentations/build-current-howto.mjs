...
add('Correct the register without losing history','REQUIREMENT CORRECTIONS',[
 ['Archive an incorrect row','Open Archive or merge requirement. Choose archive, explain why and confirm. The original stays in Archived requirements.'],
 ['Merge duplicate requirements','Choose an active target in the same pursuit and review the combined wording. Both citations are preserved; the source is archived.'],
 ['Restore for another review','Open an archived row and choose Restore requirement. The original returns as needs review. Review any merged target separately.']
],'Corrections require fresh sign-off and approval. Archiving is not a buyer waiver.','Administrators and capture managers can correct requirements. Evidence approvals and tasks remain on their original rows; merge does not transfer approval to the target. Review and link appropriate evidence to the combined requirement. Existing decisions and frozen draft snapshots stay in history, but their context becomes stale. Restore does not undo a later target edit. Archived rows are excluded from active sign-off, new outlines and active AI/research requirements. Saved response answers need reconciliation before another release. Do not archive an obligation just to remove a blocker: review the official source and document the reason.');
